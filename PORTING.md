...
| `overlay.py` | Win32 layered window (`CreateWindowExW` + `UpdateLayeredWindow`, per-pixel alpha) on a worker thread, PIL renderer; monitor work area + DPI of the focused window; timer resolution `winmm.timeBeginPeriod/timeEndPeriod`; hover detection `GetCursorPos`; click-through window styles (`WS_EX_LAYERED\|TRANSPARENT\|TOOLWINDOW\|NOACTIVATE`); font lookup via the Windows font registry |
